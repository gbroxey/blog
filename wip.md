...
Let's start by seeing in practice how it's possible to perform Lucy's algorithm in reverse. We won't worry about the Fenwick tree stuff yet, so we'll be satisfied with an $O(n^{3/4})$ runtime here. Generally it's very fast.

The main equation we rely on for prime counting (and sums of primes, and generally sums of a completely multiplicative function over primes) is

$$S_g(v, p) = S_g(v, p-1) - g(p)\left[S_g(v/p, p-1) - S_g(p-1, p-1)\right]$$

where $S_g(v, p)$ was the sum of $g(n)$ over $1 < n \leq v$ who remain after sieving out all of the composite integers with some prime factor up to $p$. Also remember that we had $S_g(v, p) = S_g(v, p-1)$ if $p$ is not a prime.
...
$$S_g(v, p-1) = S_g(v, p) + \sum_{i \geq 1, \, p^{i+1} \leq v} g(p^i) \left[S_g(v/p^i, p) - S_g(p, p)\right] + g(p^{i+1})$$
...
> 4. Next, for primes `p` in `2..sqrt(x)`, in decreasing order this time, and then for each key value `v` satisfying `v >= p*p`, perform the following:
...
$$g = u^{a_0} * N^{a_1} * N_2^{a_2} * \ldots * N_k^{a_k}$$

where, for all $n$, we define $u(n) = 1$, $N(n) = n$, and $N_i(n) = n^i$.  
The powers here are Dirichlet convolutions, for example $u^4 = u*u*u*u$.

When $a_i$ are not integers, it may get a little less nice, and could require you to take a convolution square root or something else complicated. So long as your summation limit is not unreasonable it's very simple to just use Min-25 in that case.

## Code

The code for this blog post is available link tbd.
