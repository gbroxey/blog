...
[^1]: Recall that the idea of the powerful numbers approximation trick is to find a multiplicative function $g$ such that $f(p) = g(p)$ for primes $p$, so that we have $(f/g)(p) = 0$ and $f/g$ is supported on powerful numbers which are sparse. If $f$ is multiplicative but $g$ isn't, then $f/g$ won't be multiplicative and we're very sad.[^2]

[^2]: But actually, if you instead choose $g(n) = 1$ if $n \equiv 1 \bmod 4$ and $g(n) = z$ for $n \equiv 3 \bmod 4$, and zero otherwise, where $z$ is a formal variable.. then $g(n)$ is multiplicative so long as $z^2 = 1$. So we can work in $\mathbb Z / \langle z^2 - 1 \rangle$. This is a really cool idea but it's not what we're going to pursue today.